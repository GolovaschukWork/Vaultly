...
import { RoomAccessGate } from '@/components/room-access-gate';
...
export function RoomExplorer({ roomId, folderId }: RoomExplorerProps) {
  ...
  const purgeFile = trpc.file.purge.useMutation();
  ...
  const isFilterEmpty = !isLoading && items.length > 0 && filteredItems.length === 0;
  const emptyVariant = folderId ? 'folder' : 'room';

  return (
    <RoomAccessGate roomId={roomId}>
      <RoomDndProvider roomId={roomId}>
        <AppShell
          sidebar={<TreeSidebar roomId={roomId} currentFolderId={folderId ?? undefined} />}
        ...
        >
          ...
          {isEmpty ? (
            ...
            <>
              ...
              {isFilterEmpty ? (
                <div className="border-border bg-surface-elevated/50 rounded-xl border border-dashed px-6 py-16 text-center">
                  <p className="text-content-primary mb-1 font-medium">{t('common:emptyFilter')}</p>
                  <p className="text-content-secondary text-sm">
                    {t('common:emptyFilterDescription')}
                  </p>
                </div>
              ) : (
                <FileExplorer
        ...
        <CreateFolderDialog
          ...
          onSubmit={(name) =>
            createFolder.mutate({ name, dataRoomId: roomId, parentId: folderId ?? undefined })
          }
